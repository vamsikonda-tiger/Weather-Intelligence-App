      ...
      <div className="flex items-center gap-3 overflow-x-auto pb-3 pt-1 scrollbar-thin">
        {hourly.map((item, idx) => {
          ...
          return (
            <div
              key={`${item.time}-${idx}`}
              className="p-4 bg-slate-950/80 rounded-[1.5rem] border border-slate-800/80 flex flex-col items-center justify-between min-w-[95px] shrink-0 hover:border-indigo-500/50 hover:bg-slate-900 transition-all cursor-pointer group"
            >
              <span className="text-xs font-bold text-slate-300">
                {idx === 0 ? 'Now' : item.hourLabel}
              </span>

              <div className="my-2.5 p-2 bg-slate-900 rounded-2xl border border-slate-800/80 group-hover:scale-110 transition-transform">
              ...
              {item.precipitationProbability > 0 ? (
                <div className="flex items-center gap-1 mt-1 text-[10px] font-bold text-sky-400 bg-sky-500/10 px-2 py-0.5 rounded-full border border-sky-500/20">
                  <Droplets className="w-2.5 h-2.5" />
                  <span>{item.precipitationProbability}%</span>
                </div>
              ) : (
                <div className="text-[10px] font-semibold text-slate-600 mt-1">Dry</div>
              )}
            </div>
          );
        })}
